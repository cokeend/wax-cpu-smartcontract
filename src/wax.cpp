#include </wax/mycontracts/wax/include/wax.hpp>
...
[[eosio::action]] 
void wax::modpackage( uint64_t d1, uint64_t d3, uint64_t d7, uint64_t d15 , uint64_t d30) {
   require_auth(get_self());
...
void wax::unstake(uint64_t status) {
   ...
   require_auth(get_self());
        ...
        if(status == 1){
         ...
         t.actions.emplace_back( permission_level{get_self(), "active"_n},
        get_self(),
        "run"_n,
        std::make_tuple());
